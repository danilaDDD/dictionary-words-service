...
class RestWordService:
    ...
    async def create_word(self, user_id: int, request: CreateWordRequest) -> WordResponseEntity:
        try:
            ...
            obj_id = await self.session.words.create(word)
            word = await self.session.words.find_by_id(obj_id)
    ...
    async def update_word(self, user_id: int, word_id: str,
        ...
        word = await self.session.words.find_by_id(id_obj)
        ...
        await self.session.words.update(id_obj, word)
        saved_word = await self.session.words.find_by_id(id_obj)

        return WordResponseEntity.of(saved_word)

    async def get_words(self, user_id: int) -> List[WordResponseEntity]:
        words = await self.session.words.find_by_user_id(user_id)
    ...
    async def delete_word(self, user_id: int, word_id: str) -> WordResponseEntity:
        ...
        await self.session.words.delete_by_id(id_obj)
